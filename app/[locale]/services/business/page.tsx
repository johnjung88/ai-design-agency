...
const subs: SubService[] = [
  { num: "01", name: "사업계획서", nameEn: "business plan", desc: "투자 받기 위한 IR Deck부터 사업 제안서까지", priceMan: "49", priceCheon: "", days: "5-10 DAYS", scope: "IR Deck · 사업 제안서 · 투자 발표" },
  { num: "02", name: "PPT", nameEn: "presentation", desc: "발표·세미나·제안 — 한 장이 결정짓는 자리", priceMan: "4", priceCheon: "", days: "1-3 DAYS", scope: "발표 · 세미나 · 제안서" },
  { num: "03", name: "정부지원금", nameEn: "gov. funding", desc: "K-스타트업·중기부·지자체 지원사업 서류 일체", priceMan: "99", priceCheon: "", days: "7-14 DAYS", scope: "K-스타트업 · 중기부 · 지자체" },
];

const steps = [
  { step: "01", title: "의뢰", desc: "사업·심사 기준·기한을 한 번에" },
  ...
  return (
    <section className="max-w-[1400px] mx-auto relative" style={{ padding: "calc(var(--space-section) + 40px) var(--space-edge) var(--space-section)" }}>
      ...
      <div className="inline-flex flex-wrap gap-3">
        <Link href={`/${locale}/quote`} className="inline-flex items-center gap-2 px-7 py-3 transition-all hover:-translate-y-0.5" style={{ background: "var(--tone-consult-navy)", color: "var(--tone-consult-paper)", fontFamily: "var(--font-pretendard)", fontSize: 14, fontWeight: 600, borderRadius: 2 }}>
          견적 문의 →
        </Link>
        <Link href="#subs" className="inline-flex items-center gap-2 px-7 py-3 transition-all hover:-translate-y-0.5" style={{ background: "transparent", color: "var(--tone-consult-ink)", border: "1px solid var(--tone-consult-line-2)", fontFamily: "var(--font-pretendard)", fontSize: 14, fontWeight: 500, borderRadius: 2 }}>
          sub 분야 보기
        </Link>
      </div>
    </section>
...
function SubCard({ s }: { s: SubService }) {
  return (
    <div className="group transition-all hover:-translate-y-1" style={{ padding: "clamp(24px, 2.8vw, 36px)", background: "var(--tone-consult-paper-2)", border: "1px solid var(--tone-consult-line)", borderRadius: 2 }}>
      ...
      <p className="mb-5" style={{ fontFamily: "var(--font-pretendard)", fontSize: "var(--text-body)", lineHeight: 1.7, color: "var(--tone-consult-ink-2)" }}>{s.desc}</p>
      <div className="mb-5 pb-5" style={{ fontFamily: "var(--font-ibm-plex-mono)", fontSize: 11.5, color: "var(--tone-consult-gold)", letterSpacing: "0.06em", borderBottom: "1px dashed var(--tone-consult-line)" }}>{s.scope}</div>
      <div className="flex items-baseline justify-between gap-3 flex-wrap">
        <span style={{ fontFamily: "var(--font-pretendard)", fontSize: "var(--text-body)", fontWeight: 600, color: "var(--tone-consult-ink)" }}>
          <span style={{ fontFamily: "var(--font-inter)", fontSize: "1.45em", fontWeight: 700, color: "var(--tone-consult-navy)" }}>{s.priceMan}</span>
          <span style={{ marginLeft: 2 }}>만</span>
          {s.priceCheon && (<>{" "}<span style={{ fontFamily: "var(--font-inter)", fontSize: "1.45em", fontWeight: 700, color: "var(--tone-consult-navy)" }}>{s.priceCheon}</span><span style={{ marginLeft: 2 }}>천</span></>)}
  ...
  return (
    <section className="max-w-[1400px] mx-auto" style={{ padding: "var(--space-section) var(--space-edge)" }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-3 md:gap-4">
        {steps.map((p) => (
          <div key={p.step} style={{ padding: "clamp(20px, 2.5vw, 28px)", background: "var(--tone-consult-paper-2)", border: "1px solid var(--tone-consult-line)", borderRadius: 2 }}>
            ...
            <h3 className="mb-2" style={{ fontFamily: "var(--font-inter)", fontSize: "var(--text-h3)", color: "var(--tone-consult-ink)", fontWeight: 600, lineHeight: 1.2 }}>{p.title}</h3>
  ...
  return (
    <section className="max-w-[1100px] mx-auto" style={{ padding: "var(--space-section) var(--space-edge)" }}>
      ...
      <div className="space-y-2">
        {faqs.map((f, i) => (
          <details key={i} className="group" style={{ padding: "clamp(16px, 2vw, 24px) clamp(20px, 2.5vw, 28px)", background: "var(--tone-consult-paper-2)", border: "1px solid var(--tone-consult-line)", borderRadius: 2 }}>
            <summary className="cursor-pointer flex items-center justify-between gap-3" style={{ fontFamily: "var(--font-pretendard)", fontSize: "var(--text-body)", fontWeight: 500, color: "var(--tone-consult-ink)", listStyle: "none" }}>
              <span>{f.q}</span>
              <span className="transition-transform group-open:rotate-45" style={{ fontFamily: "var(--font-inter)", color: "var(--tone-consult-navy)", fontSize: 22, lineHeight: 1, fontWeight: 300 }}>+</span>
            </summary>
            <p className="mt-4" style={{ fontFamily: "var(--font-pretendard)", fontSize: "var(--text-body)", lineHeight: 1.8, color: "var(--tone-consult-ink-2)" }}>{f.a}</p>
          </details>
        ))}
      </div>
    </section>
...
function CtaSection({ locale }: { locale: string }) {
  return (
    <section className="max-w-[1400px] mx-auto text-center" style={{ padding: "var(--space-section) var(--space-edge)", borderTop: "1px solid var(--tone-consult-line)" }}>
      <h2 className="font-normal mb-6 md:mb-8" style={{ fontFamily: "var(--font-inter)", fontSize: "var(--text-display)", lineHeight: "var(--leading-display)", color: "var(--tone-consult-ink)", fontWeight: 600, letterSpacing: "-0.018em" }}>
        결정짓는 한 장이<br />필요하신가요?
      </h2>
      ...
      <div className="inline-flex flex-wrap gap-3 justify-center">
        <Link href={`/${locale}/quote`} className="inline-flex items-center gap-2 px-8 py-4 transition-all hover:-translate-y-0.5" style={{ background: "var(--tone-consult-navy)", color: "var(--tone-consult-paper)", fontFamily: "var(--font-pretendard)", fontSize: 14, fontWeight: 600, borderRadius: 2 }}>
          견적 문의 →
        </Link>
